...
def main():
    ...
    for i, (lr_image, hr_image) in enumerate(valid_loader):
        ...
        print(f"{'='*60}")
        lr_image_noisy = add_awgn(lr_image.squeeze(0), noise_level)
        

        psnr, ssim_value, lpips_value = train_siren_for_one_image(lr_image_noisy, hr_image.squeeze(0), writer, i)
